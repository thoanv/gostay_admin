...
class TourDetails extends Component {
  ...
  static propTypes = {
    ...
    tourDate: PropTypes.array,
    tourPlan: PropTypes.array,
    open: PropTypes.bool,
  };
  ...
  render() {
    const { onOrderClose, open, infoBasic, tourDate, tourPlan, listImg } = this.props;

    const { TabPane } = Tabs;

    const columnsDate = [
      {
        title: <IntlMessages id="tour.start_date" />,
        key: "start_date",
        render: (record) => {
      ...
      {
        title: <IntlMessages id="tour.end_date" />,
        key: "end_date",
        render: (record) => {
      ...
      {
        title: <IntlMessages id="tour.price_child" />,
        key: "price_child",
        render: (record) => {
      ...
      {
        title: <IntlMessages id="tour.slot" />,
        key: "slot",
        render: (record) => {
          return record.slot;
        },
      ...
      {
        title: <IntlMessages id="tour.cost" />,
        key: "cost",
        render: (record) => {
          return record.cost;
        },
    ...
    const columnsPlan = [
      {
        title: <IntlMessages id="tour.plan.date" />,
        key: "date",
        render: (record) => {
          return record.date || '';
        },
      ...
      {
        title: <IntlMessages id="tour.plan.place_from" />,
        key: "place_from",
        render: (record) => {
      ...
      {
        title: <IntlMessages id="tour.plan.title" />,
        key: "title",
        render: (record) => {
          return record.title || '';
        },
      ...
      {
        title: <IntlMessages id="tour.plan.content" />,
        key: "content",
        render: (record) => {
          const content = JSON.parse(record.content)
          return <div>
            {content.map((item, index) => <Row key={index}>
      ...
      <React.Fragment>
        ...
        >
          ...
          <Tabs defaultActiveKey="1">
              ...
              <Row>
                <Col span={12}>
                  <Row style={this.cssRow()}>
                    ...
                    <Col span={17}>
                      <p>{infoBasic && this.getTourStatus(infoBasic.status)}</p>
                    </Col>
                  </Row>
                </Col>
                <Col span={12}>
              ...
              <Row>
                <Col span={12}>
                  <Row style={this.cssRow()}>
                    <Col span={7}>
                      <p>
                        <IntlMessages id="tour.consists_of" />:
                      </p>
                    </Col>
                    <Col span={17}>
                      <p>
                ...
                <Col span={12}>
                  <Row style={this.cssRow()}>
                    <Col span={7}>
                      <p>
                        <IntlMessages id="tour.un_consists_of" />:
                      </p>
                    </Col>
                    <Col span={17}>
                    <p>
              ...
              <Row>
                <Col span={12}>
                  <Row style={this.cssRow()}>
                    <Col span={7}>
                      <p>
                        <IntlMessages id="tour.price" />:
                      </p>
                    </Col>
                    <Col span={17}>{infoBasic ? infoBasic.price : ""}</Col>
                  </Row>
                </Col>
              </Row>
              {/* Tour days - nights */}
            ...
            <TabPane tab={<IntlMessages id="tour.date" />} key="2">
              <Row>
                <Col span={24}>
                  <Table
                    columns={columnsDate}
            ...
            <TabPane tab={<IntlMessages id="file.image" />} key="3">
              <div style={{ display: 'flex', width: '100%', flexWrap: 'wrap' }}>
                {listImg && listImg.length > 0 && listImg.map((item, index) => <div key={index} style={{ margin: 2 }}>
                  <Zoom>
                  <img
                    src={item.img_link}
                    width="280"
