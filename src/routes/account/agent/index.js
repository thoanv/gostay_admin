        ...
        const columns = [
            {
                ...
                render: (text, record) => (
                    record.image ?
                        <AvatarInTable src={config.url_asset_root + record.image}
                            defaul={record.image === '//logo3.png' ? 1 : 0}
                            title={record.firstname ? `${record.firstname}${record.lastname}` : `user`}
                            alt={`${record.firstname} ${record.lastname}`}></AvatarInTable>
                        :
                        <AvatarInTable src={require('../../../assets/img/user.png')} alt={`user`} title={record.firstname ? `${record.firstname}${record.lastname}` : `user`}></AvatarInTable>
                )
            },
            {
            ...
            },
            {
                ...
                key: 'email',
                render: (text, record) => (
                    <Button type="link" className="p-0" onClick={() => this.onEditAccount(record)}>{record.email}</Button>
                )
            },
            {
                ...
                <div className="formelements-wrapper">
                    ...
                    <div className="row">
                        <RctCollapsibleCard colClasses='col-12'>
                            ...
                            >
                                {hasSelected ? <IntlMessages id="global.selected_items" values={{ count: selectedRowKeys.length }} />: ''}
